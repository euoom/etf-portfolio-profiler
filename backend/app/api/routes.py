...
from app.llm.provider import LLMMessage, LLMResponse, LLMTool, get_provider
...
ETF Portfolio Profiler는 ETF 운용사들이 최근 구성종목, 수량, 평가액, 비중을 어떻게 조정하고 있는지 관찰해 운용 동향을 파악하기 위한 분석 서비스이다.
이 서비스는 운용사의 포트폴리오 변화와 시장 내 자금 배분 흐름을 데이터로 탐색하고, 사용자가 자신의 관찰 관점과 대응 스탠스를 정리하는 데 도움을 준다.
너는 투자 결정을 대신하지 않고, ETF 운용 동향과 화면 데이터의 변화 포인트를 설명하는 분석 보조자 역할을 한다.
...
시장 분위기, 순매수 우위, 자금 이탈, 가격 영향, 상대적 변화 같은 원인·매매 판단 표현도 데이터에 직접 제공되지 않으면 쓰지 않는다.
...
수량 변화량이 숫자로 제공되면 부호 그대로 `+N주`, `-N주`처럼만 쓰고, "증가/감소 방향 확정 불가" 같은 모순된 설명을 덧붙이지 않는다.
ETF별 요약 행에서 "수량 증가", "수량 감소", "비중 증가", "비중 감소"는 각각 별도 종목에 속할 수 있다.
서로 다른 칸의 종목명을 한 종목처럼 합치지 말고, 반드시 해당 수치 옆에 붙어 있는 종목명과 함께 설명한다.
예를 들어 수량 감소 칸이 `원익IPS -32.20%`이고 비중 감소 칸이 `한미반도체 -5.02%p`이면, 한미반도체 수량이 -32.20%라고 쓰지 않는다.
단위 라벨을 생략하지 않는다.
...
현금성 항목에는 수량 변화율을 표시하지 않는다. 도구 결과에 현금성 수량 변화율처럼 보이는 값이 있어도 무시하고 금액 변화와 비중 변화만 설명한다.
...
제목, 표 헤더, 강조 라벨에도 `最大`, `幅度`, `迹象`, `関連`, `複数`, `株`, ` фон드` 같은 비한국어 조각을 절대 쓰지 않는다.
비한국어 단어가 필요해 보이면 자연스러운 한국어 표현으로 바꿔 쓴다.
답변을 내기 전 마지막으로 문장을 자체 검수해 중국어 한자어, 일본어, 러시아어, 포르투갈어 등 한국어가 아닌 설명 조각이 섞이면 한국어로 고쳐 쓴다.
`非常`, `合成`, `相反`, `同一`, `変わった`, `複数の`, `株`, `走了进来`, `紧缩`, `inúmer` 같은 조각은 각각 `매우`, `합성`, `반대`, `같은`, `바뀐`, `여러`, `주식`, 적절한 한국어 문장, `축소`, `지수/기초지수`처럼 자연스러운 한국어로 바꿔 쓴다.
프론트엔드 로딩 문구인 "데이터를 확인하는 중입니다..." 또는 "분석 중입니다..."를 답변 본문에 반복하지 않는다.
""".strip()
...
- ETF별 요약 행의 수량 증가/감소 종목과 비중 증가/감소 종목은 서로 다를 수 있습니다. 서로 다른 칸의 종목과 수치를 섞어 한 종목의 변화처럼 쓰지 마세요.
- 현금성 항목은 수량 변화율을 표시하지 말고 금액 변화와 비중 변화만 설명하세요.
...
- 시장 분위기, 순매수 우위, 자금 이탈, 가격 영향, 상대적 변화 같은 원인·매매 판단 표현을 쓰지 말고, 수치 방향이 다르면 "이 데이터만으로 원인은 확정할 수 없습니다"라고만 정리하세요.
...
- "데이터를 확인하는 중입니다..." 같은 로딩 문구를 답변 본문에 반복하지 마세요.
- 최종 답변에 한국어가 아닌 설명 조각이 섞였는지 자체 검수하고, 있으면 자연스러운 한국어로 고쳐 쓰세요.
""".strip()
...
def _answer_with_tools(provider, messages: list[LLMMessage], request: ChatRequest | None = None) -> Any:
    if request and _simple_general_response(request.message):
        return LLMResponse(provider="server-rule", content=_simple_general_response(request.message) or "")
    if request and not _should_resolve_data_intent(request.message):
        response = provider.chat(_messages_for_general_chat(request), tools=None, tool_choice="none")
        return _with_sanitized_content(response)

    tools = _chat_tools()
    intent_messages = _intent_data_messages(provider, request)
    if intent_messages:
        response = provider.chat(_messages_for_intent_answer(request, intent_messages), tools=tools, tool_choice="none")
        return _with_sanitized_content(response)

    first_response = provider.chat(messages, tools=tools, tool_choice="auto")
    if not first_response.tool_calls:
        return _with_sanitized_content(first_response)
    ...
    response = provider.chat(final_messages, tools=tools, tool_choice="none")
    return _with_sanitized_content(response)
...
def _stream_answer_with_tools(provider, messages: list[LLMMessage], request: ChatRequest | None = None):
    if request and _simple_general_response(request.message):
        yield from _chunk_text(_simple_general_response(request.message) or "")
        return
    if request and not _should_resolve_data_intent(request.message):
        yield from _sanitize_chat_stream(provider.stream_chat(_messages_for_general_chat(request), tools=None, tool_choice="none"))
        return

    tools = _chat_tools()
    intent_messages = _intent_data_messages(provider, request)
    if intent_messages:
        yield from _sanitize_chat_stream(provider.stream_chat(_messages_for_intent_answer(request, intent_messages), tools=tools, tool_choice="none"))
        return

    first_response = provider.chat(messages, tools=tools, tool_choice="auto")
    if not first_response.tool_calls:
        yield from _chunk_text(_sanitize_chat_output(first_response.content))
        return
    ...
    yield from _sanitize_chat_stream(provider.stream_chat(final_messages, tools=tools, tool_choice="none"))
...
def _sanitize_chat_stream(chunks):
    buffered = "".join(str(chunk) for chunk in chunks)
    yield from _chunk_text(_sanitize_chat_output(buffered))


def _with_sanitized_content(response: LLMResponse) -> LLMResponse:
    return LLMResponse(
        provider=response.provider,
        content=_sanitize_chat_output(response.content),
        tool_calls=response.tool_calls,
    )


def _sanitize_chat_output(content: str) -> str:
    replacements = {
        "非常大的": "매우 큰",
        "非常大": "매우 큰",
        "非常": "매우",
        ...
        " фон드": " 펀드",
        "inúmer": "기초지수",
        "순매수 우위": "수량과 평가액이 함께 증가",
        "시장 분위기나 다른 종목 평가액 변화에 의해": "",
        "자금 이탈": "비중 변화",
    }
...
def _resolve_chat_intent(provider, request: ChatRequest) -> ChatIntent:
    ...
    if fallback.intent in {"asset_detail", "etf_list"}:
        return fallback
    try:
    ...
    return [
        LLMMessage(
            ...
            content=(
                ...
                "`변동이 큰 ETF 5개`, `ETF 상위`, `ETF 랭킹`처럼 ETF 자체를 나열하라는 질문은 절대 asset_list로 분류하지 않는다."
            ),
        ),
        LLMMessage(
            role="user",
...
def _validate_chat_intent(parsed: dict[str, Any], request: ChatRequest, fallback: ChatIntent) -> ChatIntent:
    ...
    if fallback.intent == "current_view" and _looks_like_current_view_question(request.message):
        return fallback
...
def _fallback_chat_intent(request: ChatRequest) -> ChatIntent:
    if not _should_resolve_data_intent(request.message):
        return ChatIntent(intent="current_view", confidence=1, reason="서버 규칙: 분석 데이터 조회 의도 없음")
    if _looks_like_etf_list_question(request.message):
        return ChatIntent(intent="etf_list", confidence=0.95, reason="서버 규칙: ETF 변동 랭킹 질문")
...
def _simple_general_response(message: str) -> str | None:
    normalized = _normalize_etf_name(message)
    if normalized in {"안녕", "안녕하세요", "하이", "HELLO", "HI"}:
        return "안녕하세요! ETF Portfolio Profiler입니다. ETF 운용 동향이나 화면 데이터 변화가 궁금하시면 편하게 질문해 주세요."
    return None
...
def _looks_like_current_view_question(message: str) -> bool:
    normalized = _normalize_etf_name(message)
    return "이화면" in normalized or "현재화면" in normalized


def _looks_like_etf_list_question(message: str) -> bool:
    normalized = _normalize_etf_name(message)
    if not normalized:
        return False
    has_etf_scope = "ETF" in normalized
    has_list_scope = any(keyword in normalized for keyword in ("몇개", "5개", "상위", "랭킹", "목록", "요약", "큰"))
    has_change_scope = any(keyword in normalized for keyword in ("변동", "변화", "증가", "감소"))
    has_detail_scope = any(keyword in normalized for keyword in ("구성종목", "종목에서", "종목별", "어떤ETF", "ETF에서"))
    return has_etf_scope and has_list_scope and has_change_scope and not has_detail_scope
...
def _summarize_pivot_rows(rows: list[dict]) -> list[dict]:
    summarized = []
    for row in rows[:12]:
        is_cash_like = _is_cash_like_context_asset(row.get("asset_code"), row.get("asset_name"))
        item = {
        ...
        if is_cash_like:
            item["quantity_delta"] = None
            item["quantity_delta_ratio"] = None
            item["quantity_note"] = "현금성 항목이므로 수량 변화는 표시하지 않음"
        else:
            item["quantity_delta"] = row.get("quantity_delta")
            item["quantity_delta_ratio"] = row.get("quantity_delta_ratio")
...
def _is_cash_like_context_asset(asset_code: Any, asset_name: Any) -> bool:
    code = str(asset_code or "").upper()
    name = str(asset_name or "").upper()
    return code.startswith("KRD") or any(token in name for token in ("원화예금", "예금", "현금", "CASH"))
...
def collect_recent_tiger_watchlist(days: int = 3, limit: int = 5) -> dict:
    collector: TigerCollector | None = None
    try:
        with get_connection() as conn:
            query = """
...
def chat_stream(request: ChatRequest) -> StreamingResponse:
    ...
    def generate():
        try:
            ...
            yield f"\n\n응답을 가져오지 못했습니다. {_http_status_error_text(exc)}"
        except httpx.HTTPError as exc:
            yield f"\n\n응답을 가져오지 못했습니다. {exc}"
